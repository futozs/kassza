...
<Callout type="warning" title="Az x-forwarded-for fejlécet a kliens is beállíthatja">
  Az `isSzamlazzIp()` a fejléc első címét vizsgálja. Ez csak olyan proxy mögött megbízható, amely
  felülírja a fejlécet, például Vercelen. Cloudflare mögött a `cf-connecting-ip` fejlécet add át.
  A recept ezért nem az értesítés összegeiben bízik, hanem a lekérdezett számlában.
</Callout>
