...
<Callout type="warning" title="Csak megbízható proxy mögött">
  Az `x-forwarded-for` fejlécet a kliens is beállíthatja, és az `isSzamlazzIp()` a fejléc első
  címét vizsgálja. A fenti példa Vercelen biztonságos, mert a Vercel felülírja ezt a fejlécet. A
  Cloudflare és az nginx szokásos beállítása viszont csak hozzáfűz, ezért ott a `cf-connecting-ip`,
  illetve a `proxy_set_header X-Real-IP $remote_addr` után az `x-real-ip` fejlécet ellenőrizd.
  Platformonkénti táblázatot az [IPN](/docs/befizetes-rogzitese/ipn) oldalon találsz.
</Callout>
